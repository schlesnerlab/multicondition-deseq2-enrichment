...
if (exists("snakemake")) {
  ...
  conf <- yaml::read_yaml("./configs/tec_aplnrKO.yaml")
...
if (nrow(small_joined_df) > 1) {
  ...
  diffexp_heatmap <- Heatmap(head(scaled_diffexp[order(
...
plot_pathways <- function(fgsea_res, pathways_list, gene_ranks) {
  ...
  p <- fgsea::plotGseaTable(
    pathways = pathways_list[top_pathways], stats = gene_ranks,
    fgseaRes = fgsea_res,
    gseaParam = 0.2, render = FALSE
  )
  p
}
...
save_dotplots <- function(d_plot, p_group, p_path = plot_path, gsea_type) {
  ggplot2::ggsave(filename = file.path(p_path,
                              glue::glue("{p_group}_{gsea_type}_dplot.svg")),
         d_plot, width = 10, height = 10)
}
cat("### ", "msigdb gsea", " \n")

x <- do_gseaplots(msig_gsea, setNames(
  object = joined_df$logFoldChange,
  nm = str_extract(joined_df$gene,
    pattern = "ENS[A-Z0-9]*"
  )
), id_class = "ENSEMBL")
...
if (!is.null(msig_gsea) & nrow(msig_gsea) > 1) {
  enriched_plot <- plot_enrichment(msig_gsea@result,
    X = "ID", Y = "NES",
    pval = "p.adjust"
  )
  ggsave(filename = file.path(plot_path, paste0(
    ...
    contrast_groups[2],
    "_msig_gsea_enrichplot.svg"
  )),  plot = enriched_plot)
  plot(enriched_plot)
  f_changes <- ensembl_gene_list
...
if (!is.null(msig_enrichment)) {
  do_gseaplots(msig_enrichment,
    setNames(
      object = as.data.frame(DEgenes)[, 3],
      nm = as.data.frame(DEgenes)[, 1]
    ),
    id_class = "ENSEMBL"
  ) %>% purrr::map(plot)

  if (nrow(msig_enrichment) > 1) {
    msig_enrichment <- clusterProfiler::setReadable(
      msig_enrichment,
      org_db, "ENSEMBL"
    )
...
if (!is.null(msig_enrichment)) {
  if (nrow(msig_enrichment) > 1) {
    cnetplot(
      msig_enrichment,
      foldChange = setNames(object = joined_df$logFoldChange,
                            nm = as.data.frame(gene_list)[, 1]),
      node_label = "all",
      layout = "nicely",
      cex_label_category = 0.8,
      cex_label_gene = 0.6
...
```{r KEGG plots, fig.width = 10,fig.height =10, eval = !is.null(kegg)}
library(dplyr)
dupfree_gene_list <- gene_list[!duplicated(gene_list$gname), ]

if (nrow(kegg@result) > 1) {
  kegg <- clusterProfiler::setReadable(kegg, org_db, "ENTREZID")

  p <- plot_enrichment(kegg@result,
    X = "Description", Y = "NES",
    pval = "p.adjust", pval_threshold = 0.1
  ) +
    ggtitle(glue::glue("KEGG {contrast_groups[1]}_vs_{contrast_groups[2]}"))
  ggsave(
    filename = file.path(
      plot_path,
      glue::glue("{contrast_groups[1]}_vs_{contrast_groups[2]}_KEGG_enrichment.svg")
    ),
    p
  )
  plot(p)
...
```{r fig.height=16, fig.width=16, eval = !is.null(kegg)}
if (nrow(kegg) > 1) {
  cnet_kegg <- cnetplot(
    kegg,
    foldChange = setNames(
...
if (nrow(reactome_stuff@result) > 1) {
  ...
  e_plot <- plot_enrichment(reactome_stuff@result,
    X = "Description",
    Y = "NES",
    pval = "p.adjust", pval_threshold = 0.05
  )
  ggsave(
    filename = file.path(
      plot_path,
      glue::glue("{contrast_groups[1]}_vs_{contrast_groups[2]}_reactome_enrichlpot.svg")
    ),
    e_plot
  )
  e_plot
...
if (nrow(reactome_stuff) > 1) {
  ...
  ggsave(
    filename = file.path(plot_path, paste0(
      ...
      "_vs_",
      contrast_groups[2],
      "_reactome_gsea_cnet.svg"
    )),
    reactome_cnet, width = 16, height = 16
  )
  reactome_cnet
}
```
