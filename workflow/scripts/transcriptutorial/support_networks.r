...
degree_count <- function(sif) {
  ...
  dd <- data.frame(do.call(
    rbind,
    lapply(nod, function(n) {
      # in-degree
      in_degree <- sif %>%
        dplyr::filter(Node2 == n) %>%
        nrow()
      in_positive <- sif %>%
        dplyr::filter(Node2 == n & Sign == 1) %>%
        nrow()
      in_negative <- sif %>%
        dplyr::filter(Node2 == n & Sign == -1) %>%
        nrow()
      ...
      # out-degree
      out_degree <- sif %>%
        dplyr::filter(Node1 == n) %>%
        nrow()
      out_positive <- sif %>%
        dplyr::filter(Node1 == n & Sign == 1) %>%
        nrow()
      out_negative <- sif %>%
        dplyr::filter(Node1 == n & Sign == -1) %>%
        nrow()
      ...
      r <- c(
        "node" = n, "total_count" = in_degree + out_degree,
...
getTopology <- function(networks = NULL, scafoldNET = NULL, inversCARNIVAL = F) {
  ...
  scafoldNET$target <- gsub("-", "_", scafoldNET$target,
    ignore.case = FALSE, perl = FALSE,
    fixed = FALSE, useBytes = FALSE
  )
  ...
  # set progress bar
  # pb = txtProgressBar(min = 0, max = length(networks), initial = 0, style = 3)
  ...
  for (i in 1:length(names(networks))) {
    ...
    for (edge in edges) {
      eW <- edgeWeight[edgeWeight$edge == edge, ] %>% unique()
      # if the edge exists
      if (nrow(eW) != 0) {
        if (nrow(eW) > 1) {
          eW <- arrange(eW, weight)
          eW <- eW[1, ]
        }

        topologies[edge, gsub("-", ".", sample)] <- as.numeric(eW$weight)
      } else {
        next
      }
    }
  ...
  topologies <- topologies[apply(topologies, 1, function(row) {
    any(!is.na(row))
  }), ]
