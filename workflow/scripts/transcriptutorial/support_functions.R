...
nicePCA <- function(df, targets, components = c(1, 2, 3), centering = T, scaling = F, pointSize = 4, no_label = FALSE) {
  ...
  comp1 <- ggplot(data.to.plot, aes(x = pc1)) +
  ...
  comp2 <- ggplot(data.to.plot, aes(x = pc2)) +
    geom_density(aes(alpha = 0.3)) +
    theme_minimal() +
  ...
  comp3 <- ggplot(data.to.plot, aes(x = pc3)) +
    geom_density(aes(alpha = 0.3)) +
    theme_minimal() +
  ...
  if (no_label) {
    ...
    comp1vcomp3 <- ggplot(data.to.plot, aes(x = pc1, y = pc3, color = condition)) +
      ...
      ylim(c(-max(abs(pca$x[, components[3]])), max(abs(pca$x[, components[3]])))) +
      theme(legend.position = "none") +
      scale_x_continuous(position = "top", limits = c(-max(abs(pca$x[, components[1]])), max(abs(pca$x[, components[1]]))))
...
magicPlotMaker <- function(df, outpath = ".", targets = NULL, no_pca_label = FALSE, width = 7, height = 7) {
  ...
  a <- ggplot(melted_df, aes(x = value, fill = variable)) +
    geom_density(alpha = 0.3) +
    theme_minimal()
  b <- ggplot(melted_df, aes(x = index, y = value, group = variable, color = variable)) +
    geom_point() +
    theme_minimal()
  c <- ggplot(melted_df, aes(x = index, y = value, group = variable, color = variable)) +
    geom_boxplot() +
    theme_minimal()
  ...
  for (i in 2:length(df[1, ]))
  {
    ...
    for (j in 2:length(weight_matrix[1, ]))
    {
      ...
      histo <- ggplot(sub_df, aes(x = stat, fill = "blue")) +
        ...
        theme_minimal() +
        theme(legend.position = "none", axis.text.x = element_blank(), axis.ticks.x = element_blank(), axis.title.y = element_blank(), axis.text.y = element_blank(), axis.ticks.y = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank())
...
volcano_nice <- function(df, hAss = 0.05, FCIndex, pValIndex, IDIndex, vAss = NULL,
  ...
  if (straight) {
    ...
    df$couleur <- "0"
    df$couleur <- apply(df, 1, FUN = function(x) {
      test(
        as.numeric(x[FCIndex]),
  ...
  if (label) {
    a <- ggplot(df, aes(x = logFC, y = -log(adj.P.Val), color = couleur)) +
      ...
      geom_label_repel(aes(label = condLabel)) +
      stat_function(
        fun = xneg, xlim = c(-xlimAbs, -vAss),
        color = "black", alpha = 0.7
      ) +
      ylim(c(0, ylimAbs)) +
      xlim(c(-xlimAbs, xlimAbs)) +
      stat_function(
        fun = xpos,
...
magicPlotMakerLight <- function(df, targets = NULL, no_pca_label = FALSE) {
  ...
  violins <- ggplot(melted_df, aes(x = index, y = value, group = variable, color = variable)) +
    geom_violin() +
    theme_minimal()
...
plotTF <- function(TF, geneLevelStats, regulon_df) {
  ...
  plot(ggplot(target_stats, aes(x = ID, y = stat, fill = stat)) +
