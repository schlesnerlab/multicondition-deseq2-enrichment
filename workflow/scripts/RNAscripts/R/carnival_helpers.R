...
convertHumanGeneList <- function(x, hmart, mmart) {
  genesV2 <- biomaRt::getLDS(attributes = c("hgnc_symbol"), filters = "hgnc_symbol", values = x, mart = hmart, attributesL = c("mgi_symbol"), martL = mmart, uniqueRows = T)
...
process_carnival <- function(carnival_res) {
  carnival_res$weightedSIF <- data.frame(carnival_res$weightedSIF, stringsAsFactors = F)
  carnival_res$weightedSIF$Sign <- as.numeric(carnival_res$weightedSIF$Sign)
  carnival_res$weightedSIF$Weight <- as.numeric(carnival_res$weightedSIF$Weight)

  carnival_res$nodesAttributes <- data.frame(carnival_res$nodesAttributes, stringsAsFactors = F)
  carnival_res$nodesAttributes$ZeroAct <- as.numeric(carnival_res$nodesAttributes$ZeroAct)
  carnival_res$nodesAttributes$UpAct <- as.numeric(carnival_res$nodesAttributes$UpAct)
  carnival_res$nodesAttributes$DownAct <- as.numeric(carnival_res$nodesAttributes$DownAct)
  carnival_res$nodesAttributes$AvgAct <- as.numeric(carnival_res$nodesAttributes$AvgAct)
  carnival_res
}
