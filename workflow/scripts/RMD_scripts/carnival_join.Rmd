...
if (exists("snakemake")) {
  ...
  names(carnival_paths) <- names(snakemake@config[["diffexp"]][["contrasts"]])
  contrast_names <-  names(snakemake@config[["diffexp"]][["contrasts"]])
  ...
  organism <- snakemake@config[["organsim"]]
} else {
  BASE_ANALYSIS_DIR = ''
  the_yaml <- file.path("../../configs/worflow_test.yaml")
  yaml_me <- yaml::read_yaml(the_yaml)
  carnival_paths <- ""
  names(carnival_paths) <- names(yaml_me$comp_groups)
  contrast_names <-  names(yaml_me$comp_groups)
  base_path <- ""
  tt_basepath <- ""
}
...
Here we jointly analyze the simliarities between networks.

In the first image we calulate the jaccard distance of edges between comparison groups
...
```{r, eval = T, fig.width=12, fig.height=12, eval = ifelse(length(sifts) >1, TRUE, FALSE)}
# Get common interactions in a group
...
#create_overlap_matrix(node_list)
  ...
  nodes_carnival = extractCARNIVALnodes(carnival_sample_resolution$`YECplusA-vs-YEC`)
