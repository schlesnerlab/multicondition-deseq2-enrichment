...
if (exists("snakemake")) {
  ...
  the_yaml <- yaml::read_yaml("../../../configs/VascAge_cre_config.yaml")
  ...
  diffexp_tb_path <- file.path(paste0(BASE_ANALYSIS_DIR),
...
ann_col <- colData(dds_obj) %>% as_tibble()%>% pull(!!sym(cond_id)) %>% fct_relevel(names(color_scheme))
