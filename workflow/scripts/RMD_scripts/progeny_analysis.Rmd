...
if (exists("snakemake")) {
  ...
  dds_path <- file.path(paste0(BASE_ANALYSIS_DIR), 'deseq2/rlog_transform.RDS.gz')
...
Normalized_counts<- assay(dds_obj)
...
```{r, eval = ifelse(nrow(Normalized_counts) > 1000, TRUE, FALSE)}
as_tibble(sample_acts_mat) %>% t() -> out_yeet

yeet<- apply(out_yeet, 1, kruskal.test, g = as.factor(ann_col))

kruskal_test <- purrr::map_df(yeet, tidy,.id = "pathway") %>% mutate(padj = p.adjust(p.value, method = "BH")) %>% arrange(desc(statistic))
plot_activity <- as_tibble(sample_acts_mat) %>% add_column(condition = fct_relevel(ann_col, s_groups))
