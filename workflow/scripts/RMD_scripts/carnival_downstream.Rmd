...
if (exists("snakemake")) {
  ...
  yaml <- yaml::read_yaml("../../../configs/VascAge_Apelin_config.yaml")
  comp_group <- names(yaml$diffexp$contrasts$apelin)[1]
  base_path <- yaml$dirs$BASE_ANALYSIS_DIR
  carnival_path <- file.path(
    base_path, "results/carnival/apelin",
...
plot_carn_network <- function(carn_res) {
  ed <- carn_res$weightedSIF %>% dplyr::filter(Weight >= 10)
