...
if (exists("snakemake")) {
  ...
  carnival_path <- file.path(base_path, "results/inversecarnival",
...
pathways<- loadGSC(file = dplyr::select(msig_h,
                      c("gene_symbol", "gs_name")),
                   addInfo = dplyr::select(msig_h,
...
plot_carn_network <- function(carn_res) {
  ...
  joined_nod <- left_join(nod, carn_res$nodesAttributes,
                            by = c("label" = "Node"))
  yote <- tidygraph::tbl_graph(nodes = joined_nod,
                       edges = ed
  ) %>%
  ggraph(layout = "auto") +
