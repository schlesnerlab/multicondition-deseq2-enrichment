...
if (exists("snakemake")) {
  ...
  BASE_DIR <- conf$dirs$BASE_ANALYSIS_DIR
  gsea_list <- as.list(file.path(BASE_DIR, glue::glue("results/diffexp/condition/{names(conf$diffexp$contrasts$condition)}.gseares.RDS")))
  c_groups <- names(conf$diffexp$contrasts$condition)
