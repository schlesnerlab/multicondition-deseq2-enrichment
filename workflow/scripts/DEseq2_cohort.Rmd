...
if (exists("snakemake")) {
  ...
  contrast_list <- snakemake@params[["contrast"]]
  ...
  write(paste0(
    class(diffexp_tables_paths), " and ",
    class(contrast_list)
  ), file = stderr())
  ...
  diffexp_tables_paths <- as.list(file.path(
    BASE_ANALYSIS_DIR,
    glue::glue("results/diffexp/condition/{names(conf$diffexp$contrasts$condition)}.diffexp.tsv")
  ))
...
ifelse(all(purrr::map(sig_gene_names,
                      is.null)),create_overlap_matrix(sig_gene_names), print("No enrichments present"))
```
...
for (set_n in names(H_res)) {
  if (any(H_res[[set_n]]@result$p.adjust < 0.05)) {
    ...
    cat("\n")
    cat("\n")
    ifelse(nrow(cnet_plots[[set_n]]$data) > 0, plot(cnet_plots[[set_n]]), "")
    cat("\n \n")
  }
}
```
...
df <- data.frame(SummarizedExperiment::colData(dds_obj)[, c("condition")])
rownames(df) <- colnames(SummarizedExperiment::assay(rld))
colnames(df) <- "condition"

annotation_cols <- list("condition" = group_colors)

ha <- ComplexHeatmap::HeatmapAnnotation(condition = rld@colData$condition, col = annotation_cols)
...
PCAtools::biplot(pca_data, colby = "condition", legendPosition = "right")
